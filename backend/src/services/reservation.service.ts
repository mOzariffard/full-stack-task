...
const TTL = config.reservation.ttlMinutes;
...
export const ReservationService = {
  ...
  async create(userId: string, input: CreateReservationInput): Promise<ReservationDto> {
    ...
    const existingActive = await prisma.reservation.findFirst({
      where: { userId, productId, status: ReservationStatus.PENDING },
    });
    ...
    if (existingActive) {
      ...
      await ReservationService.expireSingle(existingActive.id);
    }
    ...
    const reservation = await prisma.$transaction(
      async (tx) => {
        ...
        if (!product.is_active) throw new ConflictError('Product is not available');
    ...
    metrics.increment('reservations_created_total');
    logger.info(
      { reservationId: reservation.id, userId, productId, quantity },
      'Reservation created'
    );
  ...
  async checkout(userId: string, reservationId: string): Promise<OrderDto> {
    const order = await prisma.$transaction(
      async (tx) => {
        ...
        if (reservation.user_id !== userId) throw new NotFoundError('Reservation');
        ...
        const products = await tx.$queryRaw<
          Array<{ id: string; price: string; current_stock: number; reserved_stock: number }>
        >`
    ...
    logger.info({ orderId: order.id, userId, reservationId }, 'Order created');
  ...
  async cancel(userId: string, reservationId: string): Promise<void> {
    await prisma.$transaction(
      async (tx) => {
        const reservations = await tx.$queryRaw<
          Array<{ id: string; user_id: string; product_id: string; quantity: number; status: string }>
        >`
        ...
        if (r.user_id !== userId) throw new NotFoundError('Reservation');
        if (r.status !== 'PENDING') throw new ConflictError('Only PENDING reservations can be cancelled');
        ...
        const products = await tx.$queryRaw<Array<{ reserved_stock: number }>>`
  ...
  async expireSingle(reservationId: string): Promise<void> {
    await prisma.$transaction(async (tx) => {
      const reservations = await tx.$queryRaw<
        Array<{ id: string; product_id: string; quantity: number; status: string }>
      >`
